...
class Admin extends Base {
	...
	public function notices(){
		...
		if( $current_screen !== null && in_array( $current_screen->base, [ 'post', 'toplevel_page_templately' ] ) ) {
			return;
		}

		$global_user = Options::get_instance()->get('user', false, get_current_user_id());

		if( empty( $global_user['my_cloud']['usages'] ) || ($cloud_items = intval($global_user['my_cloud']['usages'])) <= 15 )  {
			return;
		}
