...
const Support = (function Support() {
	...
	return {
		...
		pointerEvents: !!(
			window.navigator.pointerEnabled || window.PointerEvent
		),
	...
	if (speed === 0 || !Support.transition) {
		...
		if (!swiper.animating) {
			...
			if (!swiper.onSlideToWrapperTransitionEnd) {
				swiper.onSlideToWrapperTransitionEnd = function transitionEnd$$1(
					e
				) {
		...
		prependIndexes
			...
			.forEach((index$$1) => {
				swiper.$wrapperEl.prepend(
					renderSlide(slides[index$$1], index$$1)
				);
			});
		swiper.$wrapperEl
	...
	scrollToSlide(speed, value, runCallbacks) {
		...
		if (value) {
			for (let i = 0, length = swiper.slides.length; i < length; i += 1) {
				...
				const slideHistory = History.slugify(
