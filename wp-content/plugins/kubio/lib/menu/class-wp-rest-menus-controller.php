...
class WP_REST_Menus_Controller extends WP_REST_Terms_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! current_user_can( $tax_obj->cap->edit_terms ) ) {
			if ( 'edit' === $request['context'] ) {
				return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit terms in this taxonomy.', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
			}
			return new WP_Error( 'rest_cannot_view', __( 'Sorry, you cannot view these menus, unless you have access to permission edit them. ', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'edit_term', $term->term_id ) ) {
			if ( 'edit' === $request['context'] ) {
				return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit this term.', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
			}
			return new WP_Error( 'rest_cannot_view', __( 'Sorry, you cannot view this menu, unless you have access to permission edit it. ', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	protected function check_assign_locations_permission( $request ) {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return new WP_Error( 'rest_cannot_assign_location', __( 'Sorry, you are not allowed to assign the provided locations.', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
		}
		...
		foreach ( $request['locations'] as $location ) {
			if ( ! array_key_exists( $location, get_registered_nav_menus() ) ) {
				return new WP_Error(
					...
					__( 'Invalid menu location.', 'kubio' ),
					array(
						'status'   => 400,
						'location' => $location,
					)
	...
	protected function check_set_auto_add_permission( $request ) {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return new WP_Error( 'rest_cannot_set_auto_add', __( 'Sorry, you are not allowed to set auto add pages.', 'kubio' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new WP_Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.', 'kubio' ), array( 'status' => 400 ) );
			}

			$parent = wp_get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new WP_Error( 'rest_term_invalid', __( 'Parent term does not exist.', 'kubio' ), array( 'status' => 400 ) );
			}
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			...
			return new WP_Error( 'rest_trash_not_supported', sprintf( __( "Terms do not support trashing. Set '%s' to delete.", 'kubio' ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $retval ) {
			return new WP_Error( 'rest_cannot_delete', __( 'The term cannot be deleted.', 'kubio' ), array( 'status' => 500 ) );
		}
	 ...
	 */
	public  function get_menu_auto_add( $menu_id ) {
	 ...
	 */
	public function handle_auto_add( $menu_id, $request ) {
		if ( ! isset( $request['auto_add'] ) ) {
			return true;
		}
	...
	protected function handle_locations( $menu_id, $request ) {
		...
		foreach ( $request['locations'] as $location ) {
			if ( ! in_array( $location, $menu_locations, true ) ) {
				return new WP_Error( 'invalid_menu_location', __( 'Menu location does not exist.', 'kubio' ), array( 'status' => 400 ) );
			}
	...
	public function get_item_schema() {
		...
		$schema['properties']['locations'] = array(
			'description' => __( 'The locations assigned to the menu.', 'kubio' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'string',
			),
		...
		$schema['properties']['auto_add'] = array(
			'description' => __( 'Whether to automatically add top level pages to this menu.', 'kubio' ),
			'context'     => array( 'view', 'edit' ),
			'type'        => 'boolean',
		);
